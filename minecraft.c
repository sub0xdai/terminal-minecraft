...
// These are now variables instead of macros for dynamic resizing
int Y_PIXELS = 24; // Default values
int X_PIXELS = 80;
...
// Forward declaration for free_directions
void free_directions(vect** directions);

void resize_picture(char*** picture) {
    ...
    // Allocate new memory
    *picture = malloc(sizeof(char*) * Y_PIXELS);
    for (int i = 0; i < Y_PIXELS; i++) {
...
// Function to check if terminal size has changed
int has_terminal_resized() {
    struct winsize w;
    ioctl(STDOUT_FILENO, TIOCGWINSZ, &w);
    return (w.ws_col != X_PIXELS || w.ws_row - 1 != Y_PIXELS);
}
...
// Function to free directions memory
void free_directions(vect** directions) {
    for (int i = 0; i < Y_PIXELS; i++) {
        free(directions[i]);
    }
...
int main() {
    ...
    while (1) {
        // Check for terminal resize every 10 frames
        if (frame_counter % 10 == 0) {
            if (has_terminal_resized()) {
                update_dimensions();
                resize_picture(&picture);
            }
        }
        ...
        process_input();
        if (is_key_pressed('q')) {
            break; // Use break instead of exit for proper cleanup
        }
        ...
        // Display player position and view angles
        printf("\x1B[0m\rPos: (%.1f, %.1f, %.1f) View: (%.1f, %.1f)", 
               posview.pos.x, posview.pos.y, posview.pos.z, 
               posview.view.phi, posview.view.psi);
